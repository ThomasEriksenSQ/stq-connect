...
const IGNORED_VALUES = new Set(["", "-", "n/a", "na", "ci", "cd", "none", "ukjent", "annet"]);

const IGNORED_PATTERNS = [/^communication protocols?$/i, /^kommunikasjonsprotokoller$/i, /^protocols?$/i];

const PRESERVED_COMPOUND_SEGMENTS = [/^ci\/cd$/i, /^ethernet\/ip$/i, /^tcp\/ip$/i];
