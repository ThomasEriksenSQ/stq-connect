...
import { Plus, Trash2, Pencil, X, Camera, Loader2, FileText, Upload, Move, ClipboardList, ExternalLink } from "lucide-react";
...
function SoknaderTab() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("website_applications").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    onError: () => {
      toast.error("Kunne ikke slette søknad");
      setDeletingId(null);
    },
  });
