...
export default function CVMaker() {
  ...
  return (
    <div>
      ...
      >
        {/* ── SECTION 1: HEADER ROW — logo + kontaktperson ── */}
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          {/* LOGO — centered in sidebar, padding-top 9mm */}
        ...
        {/* ── SECTION 2: PHOTO + NAME BAND ──
            Photo: starts ~12mm from top of section, 54mm tall
            Gray band: starts 20.9mm below header, 23.4mm tall
            Name: 37pt, tagline: 12pt, gap: 5.6mm, bottom padding: 4mm
        */}
          ...
          <div style={{ width: SIDEBAR_W, flexShrink: 0, display: "flex", justifyContent: "center", padding: "4mm 3mm 0 3mm" }}>
            <img
              ...
              onError={(e) => {
                ...
                div.style.cssText = "width:48mm;height:54mm;background:#919ca1;border-radius:2mm;";
                e.currentTarget.parentElement?.appendChild(div);
              }}
            />
          </div>

          {/* MAIN: white spacer then gray name band */}
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            {/* WHITE SPACER — pushes gray band to correct vertical position (20.9mm) */}
            <div style={{ height: "20.9mm" }} />

            {/* GRAY NAME BAND — 23.4mm total, contains name + gap + tagline + bottom padding */}
            <div style={{ background: "#f2f2f2", padding: "1.5mm 10mm 4mm 6mm" }}>
              {/* NAME: 37pt bold */}
              <div style={{ fontSize: "37pt", fontWeight: 700, color: "#1a1a1a", letterSpacing: "-0.01em", lineHeight: 1.1 }}>
                Mattis Spieler Asp
              </div>

              {/* TAGLINE: 12pt, 5.6mm below name */}
              <div style={{ marginTop: "5.6mm", fontSize: "12pt", color: "#444", fontWeight: 400, letterSpacing: "0.02em" }}>
                Senior Embedded-ingeniør med 8 års erfaring
              </div>
            </div>

            {/* Rest below gray band */}
            <div style={{ height: "4mm" }} />
          </div>
        </div>

        {/* ── SECTION 3: BODY — sidebar text + main content ── */}
        <div style={{ display: "flex", alignItems: "stretch" }}>
          ...
          {/* MAIN CONTENT */}
          <div style={{ flex: 1, padding: "5mm 10mm 10mm 6mm", fontSize: "9pt", lineHeight: 1.7, color: "#222" }}>
            {/* Bio */}
            {[
              ...
              <p key={i} style={{ marginBottom: "2mm" }}>{text}</p>
            ))}
            ...
            {([
              ...
              <div key={project.company + project.periode} style={{ marginBottom: "4mm", pageBreakInside: "avoid" }}>
                ...
                {project.desc.map((d, i) => (
                  <p key={i} style={{ marginBottom: "1.5mm", fontSize: "8.5pt", lineHeight: 1.65 }}>{d}</p>
                ))}
